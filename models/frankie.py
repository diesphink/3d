...
d = Dimensions()
d.bd = [59.3, 31.3, 6]
# d.parfs.distancias = [56.7, 27.8, 6]
# d.parfs.margins = [(d.bd[X] - d.parfs.distancias[X]) / 2, (d.bd[Y] - d.parfs.distancias[Y]) / 2]
d.parfs.margins = [0.75, 1.25, 0]
d.parfs.radius = 1.5
print(d.parfs.margins)
d.folga = 0.2
d.walls = 0.4
...
def four_corners(box, corner_object, margins=[0, 0, 0], zalign=None):
    alignments = [
        {"begin": "xy", "center": "", "end": ""},
        {"begin": "x", "center": "", "end": "y"},
        {"begin": "y", "center": "", "end": "x"},
        {"begin": "", "center": "", "end": "xy"},
    ]
    ...
    for alignment in alignments:
        if zalign:
            alignment[zalign] += "z"
        ret += align(corner_object, ref=box, **alignment, margins=margins)

    return ret


# %%
parf = Cylinder(radius=d.parfs.radius + d.folga / 2, height=d.cx[Z] * 10)


def punch_parf_holes(box):
    box -= four_corners(box, parf, margins=d.parfs.margins, zalign="center")

    return box


# %%
# d.first = [d.bd[X], d.bd[Y], 4]
# d.ima = [10, 10, 2]
# d.ima_margins = [3, 3, 0]

# ima = Cylinder(radius=(d.ima[X] + d.folga) / 2, height=d.ima[Z] + d.folga)

# first = Box(length=d.first[X], width=d.first[Y], height=d.first[Z])
# first = fillet(first.edges().filter_by(Axis.Z), radius=0.5)
# first -= four_corners(first, ima, margins=d.ima_margins, zalign="end")
# first = punch_parf_holes(first)

# show(first)
# export_stl(first, "library/frankie/first.stl")

d.first = [d.bd[X], d.bd[Y], 4]
d.ima = [10, 10, d.first[Z] - d.walls]
d.ima_margins = [7, 3, 0]
d.first_parf_space = [5.4, 5.4, 2.5]

ima = Cylinder(radius=(d.ima[X] + d.folga) / 2, height=d.ima[Z])
...
first -= four_corners(
    first,
    Box(width=d.first_parf_space[X], length=d.first_parf_space[Y], height=d.first_parf_space[Z]),
    margins=[0, 0, 0],
    zalign="begin",
)
first = punch_parf_holes(first)
first = first.rotate(Axis.X, 180)
...
d.second = [d.bd[X], d.bd[Y], 4]
d.corner = [
    d.parfs.margins[X] + d.parfs.margins[Y] + d.parfs.radius * 2,
    d.parfs.margins[Y] * 2 + d.parfs.radius * 2,
    4,
]

second = Box(length=d.second[X], width=d.second[Y], height=d.walls)
second = fillet(second.edges().filter_by(Axis.Z), radius=d.fillet)
second += four_corners(second, Box(length=d.corner[X], width=d.corner[Y], height=d.corner[Z]), zalign="begin")
second = punch_parf_holes(second)
show(second)

export_stl(second, "library/frankie/second.stl")

# # %%

# d.third = [d.bd[X], d.bd[Y], d.bd[Z]]
# d.corner = [
#     d.parfs.margins[X] + d.parfs.margins[Y] + d.parfs.radius * 2,
#     d.parfs.margins[Y] * 2 + d.parfs.radius * 2,
#     4,
...
# third = Box(length=d.third[X], width=d.third[Y], height=d.walls * 2)
# third = fillet(third.edges().filter_by(Axis.Z), radius=d.fillet)
# third += four_corners(third, Box(length=d.corner[X], width=d.corner[Y], height=d.corner[Z]), zalign="begin")
# third = punch_parf_holes(third)
# sidecut = Box(length=d.bd[X] * 3/4, width=d.bd[Y] * 1/4, height=d.bd[Z])
# sidecut = fillet(sidecut.edges().filter_by(Axis.Z), radius=d.fillet * 3)
# third -= align(sidecut, ref=third, center="xz", end="y", margin=-d.fillet * 3)
# third -= align(sidecut, ref=third, center="xz", begin="y", margin=-d.fillet * 3)
# show(third)

# export_stl(third, "library/frankie/third.stl")


d.third = [d.bd[X], d.bd[Y], 8]
d.corner = [
    d.parfs.margins[X] + d.parfs.margins[Y] + d.parfs.radius * 2,
    d.parfs.margins[Y] * 2 + d.parfs.radius * 2,
    8,
...
corner = Box(length=d.corner[X], width=d.corner[Y], height=d.corner[Z])
corner = fillet(corner.edges().filter_by(Axis.Z), radius=d.fillet)

third = Box(length=d.third[X], width=d.third[Y], height=d.walls * 2)
third = fillet(third.edges().filter_by(Axis.Z), radius=d.fillet)
third += four_corners(third, corner, zalign="begin")
third = punch_parf_holes(third)
sidecut = Box(length=d.bd[X] * 3 / 4, width=d.bd[Y] * 1 / 4, height=d.bd[Z] * 3)
sidecut = fillet(sidecut.edges().filter_by(Axis.Z), radius=d.fillet * 3)
# third -= align(sidecut, ref=third, center="xz", end="y", margin=-d.fillet * 3)
third -= align(sidecut, ref=third, center="xz", begin="y", margin=-d.fillet * 3)
show(third)
